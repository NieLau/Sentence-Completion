...
This experiment was created using PsychoPy2 Experiment Builder (v1.85.3),
    on Wed 04 Mar 2020 11:48:06 AM CET
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/data/hu_nieberlein/Documents/fMRI Paradigmen/SentenceCompletion/SC.psyexp',
...
text_Instruction = visual.TextStim(win=win, name='text_Instruction',
    text=u'Satzerg\xe4nzungsaufgabe:\n\nIm Folgenden sehen Sie verschiedene S\xe4tze, bei denen jeweils das letzte Wort fehlt. Bitte erg\xe4nzen Sie IM STILLEN das fehlende Wort. Denken Sie anschlie\xdfend \xfcber weitere Erg\xe4nzungsm\xf6glichkeiten nach, bis der n\xe4chste Satz auf dem Bildschirm erscheint.\n\nTeilweise sehen Sie auch Nonsense-S\xe4tze, die aus blo\xdfen Aneinanderreihungen von Buchstaben bestehen und keinen Sinn ergeben. Hier m\xfcssen Sie keine W\xf6rter erg\xe4nzen. Warten Sie in diesen F\xe4llen einfach ab, bis der n\xe4chste sinnvolle Satz erscheint.\n\nDr\xfccken Sie eine beliebige Taste, sobald Sie bereit sind, um die Aufgabe zu starten.',
    font='Arial',
...
for thisTrials_stimulu in trials_stimuli:
    ...
    # update component parameters for each repeat
    text_fixationcross.setText(fixationcross)
    # keep track of which components have finished
    fixationcrossComponents = [text_fixationcross]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if t >= 0.0 and text_fixationcross.status == NOT_STARTED:
            # keep track of start time/frame for later
            text_fixationcross.tStart = t
            text_fixationcross.frameNStart = frameN  # exact frame index
            text_fixationcross.setAutoDraw(True)
        frameRemains = 0.0 + 10.3- win.monitorFramePeriod * 0.75  # most of one frame period left
        if text_fixationcross.status == STARTED and t >= frameRemains:
